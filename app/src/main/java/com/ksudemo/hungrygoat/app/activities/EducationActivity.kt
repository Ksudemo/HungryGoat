...
import com.ksudemo.hungrygoat.app.helpers.alertDialogs.GameDialog
...
class EducationActivity : AppCompatActivity(), View.OnClickListener, DogUnboundedListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<ImageButton>(R.id.hintImgButton).setOnClickListener {
    ...
    private fun showCongratulationsDialog() {
        ...
        appConstants.win(levelCondition, 6, lastTime)
    }
    ...
    private fun setViews() {
        ...
        fun findViews() {
            exitMainMenuImgButton = findViewById(R.id.exitImgButton)
            ...
            pegButton = findViewById(R.id.pegImgButton)
            ropeButton = findViewById(R.id.ropeImgButton)
            goatButton = findViewById(R.id.goatImgButton)
            dogButton = findViewById(R.id.dogImgButton)
            eraserButton = findViewById(R.id.eraserImgButton)
    ...
    private fun isPrevStepAndCurStepAbautTieStuff(): Boolean {
        if (curStep == 0)
            return false

        val tieStuff = listOf(
            EducationStepTags.TIE_GOAT,
            EducationStepTags.TIE_DOG,
            EducationStepTags.TIE_PEG,
            EducationStepTags.TIE_ROPE
        )

        val prev = steps[curStep - 1].second
        val cur = steps[curStep].second

        Log.d("mytag", "prev step - $prev , next step - $cur")

        return prev in tieStuff && cur in tieStuff
    }
    ...
    override fun onClick(view: View?) {
        when (view?.id) {
            ...
            R.id.levelConditionImgButton -> {
                if (curStep !in steps.indices) return
            ...
            R.id.pegImgButton -> {
                if (isPrevStepAndCurStepAbautTieStuff()) return
